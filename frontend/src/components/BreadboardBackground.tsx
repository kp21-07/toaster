...
// Renders as an SVG <g> group so it composes into the parent SVG canvas in VirtualBreadboard
        ...
        <g key={`breadboard-segment-${yBase}`} transform={`translate(0, ${yBase * pitch})`}>
          ...
          <g fill="#94a3b8" fontSize="9" fontFamily="sans-serif" textAnchor="start" dominantBaseline="middle">
            {['A','B','C','D','E'].map((letter, i) => (
              <text key={`right-${letter}-${yBase}`} x={paddingX + 62 * pitch + 10} y={paddingY + (4 + i) * pitch}>{letter}</text>
            ))}
            {['F','G','H','I','J'].map((letter, i) => (
              <text key={`right2-${letter}-${yBase}`} x={paddingX + 62 * pitch + 10} y={paddingY + (11 + i) * pitch}>{letter}</text>
            ))}
            {['A','B','C','D','E'].map((letter, i) => (
              <text key={`left-${letter}-${yBase}`} x={paddingX - 15} y={paddingY + (4 + i) * pitch}>{letter}</text>
            ))}
            {['F','G','H','I','J'].map((letter, i) => (
              <text key={`left2-${letter}-${yBase}`} x={paddingX - 15} y={paddingY + (11 + i) * pitch}>{letter}</text>
            ))}
          </g>
